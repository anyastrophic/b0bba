...
class Roblox(commands.Cog, name = "roblox"):
    ...
    async def on_ready(self):
        ...
        self.verification_universe: roblox.BaseUniverse = await self.bot.verification_roblox_client.get_universe(4615409270)
    ...
    async def whois(self, interaction: discord.Interaction, user: discord.User = None):
        ...
        if registration:
            ...
            target_user_roblox = await self.roblox_client.get_user(registration['roblox_id'])

            created_on = target_user_roblox.created

            embed = discord.Embed(title = f"whois {target_user_discord.name}", colour = Enum.Embeds.Colors.Info)
            embed.add_field(name = "Roblox", value = f"**ID:** `{registration['roblox_id']}`\n**Username:** `{target_user_roblox.name}`\n**Registered On:** <t:{int(_time.mktime(created_on.utctimetuple()))}:F> `({round((_time.time() - _time.mktime(created_on.utctimetuple())) / 86400)} days)`")
    ...
    async def verify(self, interaction: discord.Interaction, username: str):
        ...
        user = await self.roblox_client.get_user_by_username(username, exclude_banned_users = True)

        if await Registration(interaction.user.id).check_registration('links') or await Registration(user.id).check_registration('links'):
            await interaction.response.send_message("Your account is already verified!")
            return

        if round((_time.time() - _time.mktime(user.created.utctimetuple())) / 86400) < 30:
            await interaction.response.send_message("I'm sorry, but you can't verify because your account's age is less than 30 days!")
            return

        verification_code = secrets.token_urlsafe(2)
        verification_store = await self.verification_universe.get_standard_datastore('Verification')
        
        try:
            await verification_store.get_entry(user.id)
            
            await interaction.response.send_message("This ROBLOX account already has a pending verification request!")
            return 
        ...
        if not isinstance(interaction.channel, discord.channel.DMChannel):
            await interaction.response.send_message("A DM has been sent to you for verification.")
...
<https://www.roblox.com/games/7646362415>
    ...
    async def _get_roles(self, interaction: discord.Interaction):
        ...
        group_member = self.group.get_member(
            link['roblox_id']
        )
    ...
    async def list(self, interaction: discord.Interaction):
        ...
        for server in servers:
            embed.add_field(name = f"{server.id}", value = f"`{server.playing} Players`", inline = False)
    ...
    async def info(self, interaction: discord.Interaction, server_id: str):
        ...
        embed.add_field(name = "Currently Playing", value = f"`{len(players)}`", inline = False)
    ...
    @commands.guild_only()
    @app_commands.checks.has_any_role(Enum.Roles.UB_Admin, Enum.Roles.UB_Junior_Admin, Enum.Roles.UB_Senior_Admin, Enum.Roles.UB_Trial_Admin)
    async def close(self, interaction: discord.Interaction, *, verdict: str):
        """close the report"""
        await Registration(interaction.user.id).admins()
        registration = await self.bot.db.admins.find_one({"discord_id": interaction.user.id})

        if interaction.channel.parent_id != 1063317564207403008:
            await interaction.response.send_message("This is the wrong channel for this command! Use it in a post in <#1063317564207403008>")
            return
        ...
        embed.add_field(name = "Report creator", value = f"<@{interaction.channel.owner_id}>", inline =False)
        embed.add_field(name = "Reported user", value = interaction.channel.name, inline =False)
        ...
        try:
            ...
            await interaction.channel.send(f"<@{interaction.channel.owner_id}>, your DMs are closed, so I pinged you for your report!")
            
        starter_message = [message async for message in interaction.channel.history(limit=1, oldest_first=True)][0]
        ...
        Logger.Reports.ReportClosed(
            interaction.channel_id,
            interaction.user
        )

    management_commands = app_commands.Group(name = "management", description = "Commands for managers")
    ...
    async def add_admin(self, interaction: discord.Interaction, user: discord.User):
        ...
        if not self.is_payout_manager(interaction.user): return
    ...
    @management_commands.command()
    async def calculate_payout(self, interaction: discord.Interaction, admin: discord.User):
        """calculate payout for specified admin"""

        if not self.is_payout_manager(interaction.user): return

        if not await Registration(admin.id).check_registration("admins"):
            await interaction.response.send_message("This admin is not in the admin collection, please add them with the `/management add_admin` command, or wait until they have a report closed.")
            return
        ...
        await interaction.response.send_message(f"The calculated payout for this admin is:\n{registration['payout']} + {bonus} BONUS = __{registration['payout'] + bonus} Robux__")

    @management_commands.command()
    async def payout(self, interaction: discord.Interaction, admin: discord.User, amount: int, notes: str = "No notes"):
        """payout a specified admin in robux"""

        if not self.is_payout_manager(interaction.user): return

        if not await Registration(admin.id).check_registration("admins"):
            await interaction.response.send_message("This user is not in the admin collection, please add them with the `/management add_admin` command, or wait until they have a report closed.")
            return
        
        link = await check_link(admin.id)

        if not link:
            await interaction.response.send_message(f"`{admin}` is not verified in B0BBA!")
            return
        ...
        registration['payouts_received'][str(len(registration['payouts_received']))] = {"manager": interaction.user.id, "amount": amount, "notes": notes}
        ...
        result = await Request().post("https://groups.roblox.com/v1/groups/11205637/payouts", headers = {"Cookie": f'.ROBLOSECURITY={cookie}', "x-csrf-token": csrf}, json = {
        ...
        await self.bot.db.admins.update_one({"discord_id": admin.id}, {"$set": {"payout": 0, "reports_closed": registration['reports_closed'], "payouts_received": registration['payouts_received']}})

        Logger.Payout.Success(
            interaction.user,
            admin
        )
        ...
        embed.add_field(name = "Manager, who paid out", value = f"{interaction.user}", inline = False)
    ...
    @management_commands.command()
    async def get_reports_closed(self, interaction: discord.Interaction, admin: discord.User):
        ...
        for reportid, reportdata in registration['reports_closed'].items():
            if reportdata['expired'] == False:
                report = await self.bot.db.reports.find_one({
                    "report_id": reportid
                })
                
                if report:
                    sheet.append((str(reportid), str(report['creator']), reportdata['verdict']))
                else:
                    sheet.append((str(reportid), str(reportdata['creator']), reportdata['verdict']))

        book.save(f'./temp/{admin.name}.xlsx')

        with open(f"./temp/{admin.name}.xlsx", "rb") as file:
            await interaction.response.send_message(file = discord.File(file, filename = f"{admin.name}.xlsx"))
    ...
    robloxmod_commands = app_commands.Group(name = "robloxmod", description = "Commands for Roblox moderations")

    @robloxmod_commands.command()
    @commands.guild_only()
    @app_commands.checks.has_any_role(Enum.Roles.UB_Admin, Enum.Roles.UB_Junior_Admin, Enum.Roles.UB_Senior_Admin, Enum.Roles.UB_Trial_Admin, 1003365453588070552) # 1003365453588070552 is manslaughter for testing
    async def globalmessage(self, interaction: discord.Interaction, message: str = "A default admin message sent from B0BBA bot"):
        """sends a message accross every UB server"""

        verified = await check_link(interaction.user.id)

        if not verified:
            await interaction.response.send_message("You can't run this command because your account isn't verified! Run `/verify` with your ROBLOX username")
            return
        
        result = await self.universe.publish_message("globalMessage", message)

        if result == 200:
            Logger.RobloxMod.GlobalMessage.Success(
                interaction.user,
                message
            )
    ...
    @robloxmod_commands.command()
    @app_commands.checks.has_any_role(Enum.Roles.UB_Admin, Enum.Roles.UB_Junior_Admin, Enum.Roles.UB_Senior_Admin, Enum.Roles.UB_Trial_Admin, 1003365453588070552)
    ...
    async def gameban(self, interaction: discord.Interaction, target_user: str, reason: str, time: int, unit: app_commands.Choice[str]):
        """gameban a user in UB"""

        verified = await check_link(interaction.user.id)
        if not verified:
            await interaction.response.send_message("You can't run this command because your admin account isn't verified! Run `/verify` with your ROBLOX username")
            return
        ...
        await interaction.response.send_message(f'`{target_username}` succesfully gamebanned')
        await self.universe.publish_message("banUpdate", f'{target_username}:{ban_duration}:{reason}')

        channel = interaction.guild.get_channel(1056047980861476945)

        embed = discord.Embed(title = "Gamebanned a player", colour = Enum.Embeds.Colors.Info)
        ...
        thumbnails = await self.roblox_client.thumbnails.get_user_avatar_thumbnails(
                [admin], 
                roblox.AvatarThumbnailType.headshot, 
                is_circular = True
            )

        embed.set_author(
            name = admin.name, 
            icon_url = thumbnails[0].image_url
        )
    ...
    @commands.guild_only()
    @app_commands.checks.has_any_role(Enum.Roles.UB_Admin, Enum.Roles.UB_Junior_Admin, Enum.Roles.UB_Senior_Admin, Enum.Roles.UB_Trial_Admin)
    async def ungameban(self, interaction: discord.Interaction, target_user: str, reason: str):
        ...
        await interaction.response.send_message(f'`{target_username}` succesfully ungamebanned')

        channel = interaction.guild.get_channel(1056047980861476945)

        embed = discord.Embed(title = "Ungamebanned a player", colour = Enum.Embeds.Colors.Info)
        embed.add_field(name = "Reason:", value = reason, inline = False)
    ...
    async def get_ban_state(self, interaction: discord.Interaction, target_user: str):
        ...
        embed = discord.Embed(title = f"{target_username}'s ban state", colour = Enum.Embeds.Colors.Info)
    ...
    @commands.guild_only()
    @app_commands.checks.has_any_role(Enum.Roles.UB_Admin, Enum.Roles.UB_Junior_Admin, Enum.Roles.UB_Senior_Admin, Enum.Roles.UB_Trial_Admin)
    async def execute(self, interaction: discord.Interaction, server_id: str, *, command: str):
        ...
        user = await self.roblox_client.get_user(
            verified['roblox_id']
        )

        await self.universe.publish_message('global', f"{server_id}|{verified['roblox_id']}|{user.name}|/{command}")
