...
class Utility(commands.Cog, name = "util"):
    ...
    async def check_battery(self):
        ...
        if battery is not None:
            ...
            if battery.percent <= 10 and not battery.power_plugged and not self.battery_debounce:
                t = threading.Thread(
                    target = os.system,
                    args = ['ffplay alert.mp3 -autoexit -nodisp']
                )
    ...
    async def on_message(self, message):
        ...
        if message.channel.id == 1054082338268647565 and message.author.id != 1085454927259779093:
            if message.attachments:
                ...
                await thread.send(
                    'You can discuss this creation here!'
                )
        ...
        if message.channel.type == discord.ChannelType.public_thread and message.channel.parent.id == 1063317564207403008 and message.author.id != 1085454927259779093 and message == message.channel.starter_message:
            try:
                user = await self.roblox_client.get_user_by_username(message.channel.name)
                ...
                b0bba_link = await self.bot.db.links.find_one(
                    {
                        "roblox_id": user.id
                    }
                )
    ...
    async def self_role(self, interaction: discord.Interaction, role: str):
        ...
        if role.id in self.WHITELISTED_ROLES:
            ...
            await interaction.response.send_message(
                'Sorry, you can\'t have this role!'
            )
    ...
    async def _self_role_autocomplete(self, interaction: discord.Interaction, current: str) -> List[app_commands.Choice[str]]:
