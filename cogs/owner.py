...
class Owner(commands.Cog, name = 'owner'):
    ...
    async def role(self, interaction: discord.Interaction, member: discord.Member, role: discord.Role) -> None:
    ...
    @owner_commands.command()
    @commands.is_owner()
    async def blacklist(self, interaction: discord.Interaction, user: discord.User, *, reason: str = "No reason specified") -> None:
        if not is_owner(interaction.user): return
        ...
        if already_blacklisted:
            await interaction.response.send_message(f"`{user}` is already blacklisted for `{already_blacklisted['reason']}`")

        await interaction.response.send_message(f'`{user}` successfully blacklisted for `{reason}`')
    ...
    async def eval_fn(self, ctx, *, code: str):
        if not is_owner(ctx.message.author): return
