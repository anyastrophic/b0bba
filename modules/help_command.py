...
class HelpCommand():
    ...
    async def identify_command(self, command: str, tree):
        ...
        for cmd in commands:
            if type(cmd) == discord.app_commands.Group and cmd.qualified_name == command:
    ...
    async def get_group_from_commands(self, group_name, commands):
        for cmd in commands:
            if type(cmd) == discord.app_commands.Group and cmd.qualified_name == group_name:
                return cmd
    ...
    async def get_bot_help(self, cogs, tree):
        ...
        embed = discord.Embed(title = "Help", color = Enum.Embeds.Colors.Info)
        
        for cog_name, cog in cogs.items():
            value = "\t".join(f"`{i.qualified_name}`" for i in await self.get_commands_in_cog(cog_name, commands))
    ...
    async def get_command_help(self, command, tree):
        ...
        embed = discord.Embed(title = f"{command.qualified_name} help", color = Enum.Embeds.Colors.Info)
    ...
    async def get_group_help(self, group, tree):
        ...
        embed = discord.Embed(title=f"**{group.qualified_name}** subcommands", colour = Enum.Embeds.Colors.Info)
        if group.description:
            embed.description = f'{group.description}'
            
        for command in group.commands:
            embed.add_field(name=command.qualified_name, value=f"`{command.description}`" or "`No description`", inline = False)
    ...
    async def get_cog_help(self, cog, cogs):
        ...
        embed = discord.Embed(title=f"**{cog.qualified_name}** commands", colour = Enum.Embeds.Colors.Info)
        if cog.description:
