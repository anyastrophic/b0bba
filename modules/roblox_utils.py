...
class User:
    ...
    async def get_headshot(self, **kwargs):
        result = await Request().get(
            f"https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds={self.id}&size={kwargs.get('size', '720x720')}&format={kwargs.get('format', 'png')}&isCircular={kwargs.get('circle', 'false')}"
        )
    ...
    async def get_id_from_username(self, username, limit=False):
        ...
        if len(result["json"]["data"]) > 0 and limit:
            ...
            raise (RobloxUtilsError(
                Placeholder.Errors.RobloxUtils.UsernameNotFound))
...
class Place:
    ...
    async def get_servers(self):
        result = await Request().get(
            f"https://games.roblox.com/v1/games/{self.id}/servers/Public?sortOrder=Asc&limit=100"
        )
...
class Universe:
    ...
    async def publish_message(self, topic, message):
        result = await Request().post(
            f"{APIS}/messaging-service/v1/universes/{self.id}/topics/{topic}",
    ...
    async def update_data(self, datastore, key, value):
        ...
        result = await Request().post(
            f"{APIS}/datastores/v1/universes/{self.id}/standard-datastores/datastore/entries/entry?datastoreName={datastore}&entryKey={key}",
            headers={
                "x-api-key": API_KEY,
